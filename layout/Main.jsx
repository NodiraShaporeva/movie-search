...
class Main extends React.Component {
    ...
    handleEnter = (search, type) => {
        ...
        let url = `https://www.omdbapi.com/?apikey=5f29dc7d&s=${search}`;
        if (type !== 'all') {
            url = url + `&type=${type}`;
        }
        fetch(url)
    ...
    handleReadMore = (id) => {
        ...
        fetch(`https://www.omdbapi.com/?apikey=5f29dc7d&i=${id}&plot=full`)
            .then(response => response.json())
            .then(data => {
                this.setState({
    ...
    componentDidMount() {
        fetch('https://www.omdbapi.com/?apikey=5f29dc7d&s=matrix')
            .then(response => response.json())
            .then(data => {
                this.setState({
                    movies: data.Search ? data.Search : [],
